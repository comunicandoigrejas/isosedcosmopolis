...
if st.session_state.pagina == "Início":
    ...
    # 🍇 Santa Ceia Dinâmica (Busca na aba Agenda)
    df_ag_home = carregar_dados("Agenda")
    prox_ceia = "A definir"
    if not df_ag_home.empty:
        # Tenta converter as datas e filtrar por "Santa Ceia"
    ...
    # 🎂 Aniversariantes do Mês (Correção do KeyError)
    df_niver = carregar_dados("Aniversariantes")
    if not df_niver.empty:
        # Busca a coluna de mês ignorando acentos ou espaços
        col_m = next((c for c in df_niver.columns if 'mes' in c.lower() or 'mês' in c.lower()), None)
        if col_m:
            niver_mes = df_niver[df_niver[col_m] == hoje_br.month]
            if not niver_mes.empty:
                nomes = ", ".join([f"{r['Nome']} ({r['Dia']})" for _, r in niver_mes.sort_values('Dia').iterrows()])
                st.markdown(f"<p style='text-align:center; font-size:0.85em;'>🎂 <b>Aniversariantes de {calendar.month_name[hoje_br.month]}:</b><br>{nomes}</p>", unsafe_allow_html=True)
...
# =========================================================
# --- 4. OUTRAS PÁGINAS (Com Abas por Meses) ---
# =========================================================
    ...
    col_m = next((c for c in df.columns if 'mes' in c.lower() or 'mês' in c.lower()), "Mes")
    abas = st.tabs([calendar.month_name[i].capitalize()[:3] for i in range(1,13)])
    if not df.empty:
        for i, aba in enumerate(abas):
            with aba:
                mes_df = df[df[col_m] == (i+1)].sort_values('Dia')
                for _, r in mes_df.iterrows():
    ...
    if not df.empty:
        ...
        st.markdown(f'<div class="texto-biblico">{buscar_versiculo(item["Referência"])}</div>', unsafe_allow_html=True)
        st.markdown(f"**Meditação:** {item['Resumo para meditação']}")

elif st.session_state.pagina == "Devocional":
    st.button("⬅️ VOLTAR", on_click=navegar, args=("Início",))
    st.markdown("<h2>📖 Devocional (Meditar)</h2>", unsafe_allow_html=True)
