...
# IMPORTANTE: Garante que o link da planilha está aqui
...
st.markdown("""
    ...
    .btn-voltar div.stButton > button { background-color: rgba(255,255,255,0.1) !important; height: 45px !important; }
...
if st.session_state.pagina == "Início":
    ...
    if not df_d.empty:
        ...
        if not hoje.empty:
            dev = hoje.iloc[0]
            st.markdown("---")
            # Exibe o TEMA em texto pequeno (caption)
            if pd.notna(dev.get('tema')):
                st.caption(f"🏷️ TEMA: {dev['tema']}")
            
            # TÍTULO em destaque
            st.header(dev.get('titulo', 'Meditação do Dia'))
            
            # VERSÍCULO em caixa de sucesso/destaque
            st.success(f"📖 **VERSÍCULO:** {dev.get('versiculo', 'Consulte a sua Bíblia')}")
            
            # TEXTO principal
            st.write(dev.get("texto", "Conteúdo em breve..."))
            
            # Blocos extras de APLICAÇÃO e DESAFIO
            ...
            st.info(f"📅 Ainda não temos uma meditação cadastrada para o dia {d_sel.strftime('%d/%m/%Y')}.")
    else:
        st.error("Erro ao carregar a aba 'Devocional' da planilha.")

# --- OUTRAS PÁGINAS (Agenda, Departamentos, Aniversariantes, Escalas) ---
# [A lógica de carregamento dessas abas continua igual à anterior]
    ...
    with t1:
        ...
        if not df.empty:
            for _, r in df.iterrows():
                st.markdown(f'<div class="card-agenda" style="border-left-color:#00ffcc"><b>{r.get("data","")}</b><br>{r.get("culto","")}</div>', unsafe_allow_html=True)
    with t2:
        df = carregar_dados("Recepcao")
        if not df.empty:
            for _, r in df.iterrows():
                st.markdown(f'<div class="card-agenda" style="border-left-color:#ffd700"><b>{r.get("data","")}</b><br>👥 {r.get("dupla","")}</div>', unsafe_allow_html=True)
